...
export type WireguardIPv4Node = InternetSchemas.IPv4;
export type WireguardIPv6Node = InternetSchemas.IPv6;

/**
 * A server in the network can either have an ipv4 address pool or ipv6 address
 * pool.
 */
export type WireguardIPv4Server = InternetSchemas.IPv4SetupData;
export type WireguardIPv6Server = InternetSchemas.IPv6SetupData;
 ...
 */
export type WireguardNetworkNodes =
 ...
 */
export type ConnectionLayer<Nodes extends WireguardNetworkNodes> = HashMap.HashMap<
...
export type FirewallLayer<Nodes extends WireguardNetworkNodes> = HashMap.HashMap<
    Nodes[number],
...
export type WireguardNetwork<Nodes extends WireguardNetworkNodes> = {
    nodes: Nodes;
    allowedIPs: FirewallLayer<Nodes>;
    connections: ConnectionLayer<Nodes>;
    cidr: InternetSchemas.IPv4CidBlock | InternetSchemas.IPv6CidBlock;
};
        ...
        Array.map(
            (node: (WireguardIPv4Server | WireguardIPv4Node | WireguardIPv6Node | WireguardIPv6Server) & Keys) => {
                ...
                return Tuple.make(id, {
                    ...
                    Endpoint: endpoint !== undefined ? { ...endpoint, ip: endpoint.address.value } : undefined,
                });
            }
        ),
        Record.fromEntries
